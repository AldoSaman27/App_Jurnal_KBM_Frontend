...
const login = () => {
    ...
    const handleSubmit = async () => {
        if (nipValue === "" || nipValue.length < 18 || nipValue === null || nipValue === undefined) {
            return Alert.alert(
                "Opss!", 
                "NIP is required or NIP must be 18 digits!"
            );
        ...
        await axios.post(`${process.env.EXPO_PUBLIC_API_URL}/api/user/login`, formData).then((res) => {
            setIsLoading(false);
        ...
        <SafeAreaProvider>
            <View style={styles.container}>
                ...
                <Text style={styles.register_text}>
                    Tidak punya akun?{" "}
                    <Link href={"/auth/register"} style={styles.register_text_link}>Buat Akun</Link>
                </Text>
            </View>
        </SafeAreaProvider>
    );
